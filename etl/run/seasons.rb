...
def parseMatches (browser, season_name, round_name, group_name) #(matches)
  ...
  while click_away do
    ...
    #pp matches.css('span.nav_description a')
    #puts matches.css('span.nav_description a').attribute('class')
...
#url = "http://www.soccerway.com/national/turkey/super-lig/2010-2011/regular-season"
matches = nil
...
seasons.css("#season_id_selector option").each do |season|
  ...
  if browser.select_list(:id => "round_id_selector").exists? then
    puts "Rounds exists."
    ...
    #puts ln
    file.write ln    
  end #if browser.select_list(:id => "round_id_selector").exists? then
